...
const MyAppointment = () => {
  ...
  const initPay = (order)=>{
    const options={
      key:'rzp_test_v5HJ0uDk7bKXsO',
      amount: order.amount,
      currency: order.currrency,
      ...
      handler: async (response)=>{
        ...
        try {
          const {data}=  await axios.post('http://localhost:4000/api/user/verify-razorpay',response,{headers:{token}})
          if(data.success){
            getAppointment()
        ...
        {appointment.map((item,index)=>(
          <div  className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {!item.cancelled && <button onClick={()=>cancelAppointment(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded  hover:bg-red-600  hover:text-white transition-all duration-300 '>Cancel apoointment</button>}
